...
class NFC_PN532 {
  public:
    NFC_PN532(uint8_t clk, uint8_t miso, uint8_t mosi,
              uint8_t ss);                 // Software SPI
    NFC_PN532(uint8_t irq, uint8_t reset); // Hardware I2C
    NFC_PN532(uint8_t ss);                 // Hardware SPI
    void begin(void);

    // Generic PN532 functions
    bool SAMConfig(void);
    uint32_t getFirmwareVersion(void);
    bool sendCommandCheckAck_Felica(uint8_t *cmd, uint8_t cmdlen,const uint8_t *body = 0, uint8_t blen = 0,uint16_t timeout = 100);
    bool sendCommandCheckAck(uint8_t *cmd, uint8_t cmdlen,uint16_t timeout = 100);
    bool writeGPIO(uint8_t pinstate);
    uint8_t readGPIO(void);
    bool setPassiveActivationRetries(uint8_t maxRetries);

    // ISO14443A_B functions
  ...
  private:
    ...
    void writecommand_Felica(uint8_t *cmd, uint8_t cmdlen,const uint8_t *body , uint8_t blen);
    bool isready();
